    ...
    if not request.session.get('admin_login'):
        ...
        if request.method == "POST":
            ...
            if check:
                ...
                addUser = Admin(username=username, password=password, dept=branch, role="PIMS Admin", name=name,admin_type=admin_type)
                addUser.save()
                ...
                <p>Link of the website is <a href="https://tnpportal.kjsieit.in/">Pims Portal</a>

                </body>
                </html>
                """%(admin_type,username,password)
                
                email = EmailMessage('Placement',email_boby,"pimsportal@tnpportal.kjsieit.in",[username])
    ...
    if not request.session.get('admin_login'):
        ...
        if request.method == "POST":
            ...
            if not compCheck.exists():
                ...
                studentObj = Resume.objects.filter(agg__gte=eligible,any_present_kt__lte=any_live_kt, branch__in=branch)
    ...
    if not request.session.get('admin_login'):
        ...
        if(request.method =='POST'):
            ...
            """     % (comp.comp_name, comp.comp_name,comp.comp_profile, comp.ctc, comp.branch, comp.eligibility, comp.date, comp.time, comp.venue, comp.bond, comp.instruction, urllib.parse.urlencode({'token':token}), urllib.parse.urlencode({'stud':user}))
                    email = EmailMessage('Placement', email_body, 'pimsportal@tnpportal.kjsieit.in', [user])
                    email.content_subtype = "html"
                    email.send()
...
def unlockResume(request):
    if not request.session.get('admin_login'):
        ...
        if(admin_type=="Super"):
            user = request.GET.get('s')
            resume = Resume.objects.get(somaiya_id=user)
            resume.lock = False
            resume.save()
            return HttpResponseRedirect("/tnp_admin/user_display")
        else:
            user = request.GET.get('s')
            resume = Resume.objects.filter(somaiya_id=user,branch=admin_type)
            resume.lock = False
            resume.save()
            return HttpResponseRedirect("/tnp_admin/user_display")
